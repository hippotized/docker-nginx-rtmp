...
# Get nginx source and nginx-rtmp module. \
  curl -LRo - https://nginx.org/download/nginx-${NGINX_VERSION}.tar.gz | tar xz -C /tmp/ && \
  curl -LRo - https://github.com/sergey-dryabzhinsky/nginx-rtmp-module/archive/${NGINX_RTMP_VERSION}.tar.gz | tar xz -C /tmp/ && \
...
# Get FFmpeg source.
RUN curl -LRo - https://ffmpeg.org/releases/ffmpeg-${FFMPEG_VERSION}.tar.gz | tar xz -C /tmp/ && \
# Compile ffmpeg. \
  export MAKEFLAGS="-j$(getconf _NPROCESSORS_ONLN)" && \
